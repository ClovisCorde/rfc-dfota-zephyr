 ...
 * @brief Defines the states for the decoder.
 */
typedef enum {
...
struct bspatch_stream
{
	...
	size_t offset;
	int (*decompress)(struct bspatch_stream* stream, struct delta_api_t *self_p, uint8_t* output_buffer, size_t output_buffer_size, heatshrink_decoder *hsd, size_t *bytes_written, decoder_state_t *state);
};
 ...
 */
static int decompress_patch(struct bspatch_stream* stream, struct delta_api_t *self_p, uint8_t *output_buffer, size_t output_buffer_size, heatshrink_decoder *hsd, size_t *bytes_written, decoder_state_t *state) {
	...
	self_p->memory.flash.slot = STORAGE_PARTITION;
	...
	*state = STATE_SINK;
	while (true)
	{
		...
		{
		case STATE_SINK:	
			/* read INPUT_READ_SIZE bytes in the patch */
			...
			ret = self_p->seek(&self_p->memory, self_p->memory.offset.source, self_p->memory.offset.patch + INPUT_READ_SIZE);
			if (ret != 0) {
		...
		case STATE_POLL:
			...
			while (poll_res != HSDR_POLL_EMPTY && bytes_to_decode > 0) {
				size_t to_decode = output_buffer_size - offset_buffer < OUTPUT_READ_SIZE ? output_buffer_size - offset_buffer : OUTPUT_READ_SIZE;
...
static int64_t offtin(uint8_t *buf)
{
	...
	/* Start with the last byte (buf[7]) and use a mask (0x7F) to keep only the lower 7 bits (ignoring the sign bit) */
	y=buf[7]&0x7F;
	/* Accumulate the bytes into "y". Each byte is shifted left by multiplying it by 256 before adding the next byte. */
	y=y*256;y+=buf[6];
	y=y*256;y+=buf[5];
	y=y*256;y+=buf[4];
	y=y*256;y+=buf[3];
 ...
 */
static int bspatch(struct delta_api_t *delta_apply, int64_t new_size, uint8_t *patch, size_t *offset_buffer, size_t *bytes_written, struct bspatch_stream* stream, decoder_state_t *state)
{
	...
	while(newpos<new_size) {
		...
		if(*bytes_written == 0) {
			ret = stream->decompress(stream, delta_apply, patch, CONTROL_DATA_SIZE*CONTROL_DATA_NB, stream->hsd, bytes_written, state);
		...
		while (ctrl[0] > 0) {
			...
			if(*bytes_written == 0) {
				...
				/* If we decompressed less bytes than the maximum possible that means we decompressed all the data for this ctrl[0] block */
			...
			*bytes_written -= bytes_to_read;
			...
			delta_apply->memory.flash.slot = SLOT_0;
			...
			ret = delta_apply->seek(&delta_apply->memory, oldpos, delta_apply->memory.offset.patch);
			if (ret != 0) {
		...
		while(ctrl[1] > 0) {
			...
			if(*bytes_written == 0) {
				...
				ret = stream->decompress(stream, delta_apply, patch, decoded, stream->hsd, bytes_written, state);
				/* If we decompressed less bytes than the maximum possible that means we decompressed all the data for this ctrl[1] block */
				if(*bytes_written < OUTPUT_BUFFER_SIZE) {
					heatshrink_decoder_reset(stream->hsd);
				}
				*offset_buffer = 0;
			...
			*bytes_written -= bytes_to_read;
			...
			ret = delta_apply->write(&delta_apply->memory, new, bytes_to_read, newpos + bytes_to_read == new_size);
			if (ret < 0) {
				LOG_ERR("Flash write error: %d\n", ret);
				return ret;
			}
...
int bsdiff_patch(struct delta_api_t *self_p) 
{
	...
	/* Read the header of the patch (stored in flash in the storage partition) */
	self_p->memory.flash.slot = STORAGE_PARTITION;
	self_p->read(&self_p->memory, patch, PATCH_HEADER_SIZE);
    
    
	/* Check for appropriate magic in the header's patch */
	memcpy(header, patch, HEADER_BLOCK_SIZE);
	if (memcmp(header, "BSDIFFHS", HEADER_BLOCK_SIZE) != 0) {
		LOG_ERR("Wrong magic in the patch");
        return -ENOTSUP;
	}
   
	/* Read the size of the target firmware from header's patch */
	memcpy(header, patch + HEADER_BLOCK_SIZE, HEADER_BLOCK_SIZE);
    new_size = offtin(header);
    if (new_size < 0) {
        LOG_ERR("Wrong new size in the patch");
        return -ENOTSUP;
    }	
	self_p->memory.offset.new_size = new_size;

	/* Update the offset reading in the patch*/
	self_p->seek(&self_p->memory, self_p->memory.offset.source, self_p->memory.offset.patch + PATCH_HEADER_SIZE);

	ret = bspatch(self_p, new_size, patch, &offset_buffer, &bytes_written, &stream, &state);
    if (ret != 0) {
